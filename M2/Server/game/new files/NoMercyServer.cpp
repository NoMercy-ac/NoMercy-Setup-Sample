...
#error "NOMERCY_GAME_HEARTBEAT_CHECK_V1 and NOMERCY_GAME_HEARTBEAT_CHECK_V2 both defined, please enable only one of them!"
#endif
...
EVENTFUNC(nomercy_event)
{
	...
	return PASSES_PER_SEC(NOMERCY_HEARTBEAT_CHECK_INTERVAL);
}
#endif
...
void CNoMercyServer::OnLoginPlayer(const unsigned int c_nPlayerID)
{
	...
	LPEVENT pkEvent = event_create(nomercy_event, info, NOMERCY_HEARTBEAT_CHECK_INITIAL_DELAY);
